    ...
    def _parse_line(self, line, line_num):
        ...
        if m:
            ...
            if PATTERNS['sql_injection'].search(path):
                self._add_alert('CRÍTICO', f'Tentativa de SQL Injection da linha {line_num}', ip, line)
                severity = 'critical'
                event_type = 'sql_injection'
            elif PATTERNS['xss_attempt'].search(path):
                self._add_alert('ALTO', f'Tentativa de XSS da linha {line_num}', ip, line)
                severity = 'high'
                event_type = 'xss_attempt'
            elif PATTERNS['path_traversal'].search(path):
                self._add_alert('ALTO', f'Path Traversal detectado na linha {line_num}', ip, line)
                severity = 'high'
                event_type = 'path_traversal'

            if PATTERNS['suspicious_ua'].search(line):
                self._add_alert('ALTO', f'Ferramenta de ataque detectada na linha {line_num}', ip, line)
                severity = 'high'
...
def analyze():
    ...
    if 'file' in request.files and request.files['file'].filename:
        ...
        if not allowed_file(file.filename):
            return jsonify({'error': 'Formato não permitido. Use .log, .txt ou .csv'}), 400
